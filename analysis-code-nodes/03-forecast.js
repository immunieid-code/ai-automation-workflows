...
for (const row of input.cleanedRows) {
  ...
  const previous = bucket.get(period) || { total: 0, count: 0 };
  previous.total += row[plan.target]; previous.count++;
  bucket.set(period, previous);
}
...
const values = periods.map((p) => plan.forecastAggregation === 'mean' ? bucket.get(p).total / bucket.get(p).count : bucket.get(p).total);
...
input.advancedAnalysis.results.push({ method: 'forecast', status: 'ok', target: plan.target,
  frequency: 'monthly', aggregation: plan.forecastAggregation, observedPeriods: periods.length, holdoutPeriods: testCount,
