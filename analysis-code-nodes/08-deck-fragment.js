...
function resultDetails(result) {
  ...
  if (result.method === 'forecast') return `Target: ${esc(result.target)} · ${esc(result.selectedModel)} · Holdout MAE: ${esc(num(result.models.find((m) => m.model === result.selectedModel)?.mae, 3))} · ${esc(result.forecast.length)} future monthly periods: ${result.forecast.map((p) => `${esc(p.period)} ${esc(num(p.value, 2))}`).join('; ')}`;
